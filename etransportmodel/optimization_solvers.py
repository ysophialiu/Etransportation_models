...
class OptimizationSolvers(ChargingPlacement):
    ...
    def generate_initial_data_BO(self, n, ddtype):  # n is number of initial value want to generate
        train_x = self.gen_high_dimension_variable(n)
        exact_obj = ChargingPlacement.Optimization_function(train_x).unsqueeze(-1).to(ddtype)
    ...
    def get_next_points_BO(self, init_x, init_y, best_init_y, bounds_init_x, BATCH_SIZE):
        ...
        BO_bound = (torch.tensor([[0],[1]])).repeat(1,self.dim).to(self.ddtype)
    ...
    Documentation.
    """
    ### Edited all variables to remove _
    def BO_run(self, N_initial, BATCH_SIZE, N_BATCH, N_TRIALS):
        ...
        for trial in range(1, N_TRIALS + 1):
            ...
            for iteration in range(1, N_BATCH + 1):    
                ...
                new_results = ChargingPlacement.Optimization_function(new_candidates).unsqueeze(-1)
    ...
    def BO_add_iter(self, N_initial_,BATCH_SIZE_,N_BATCH_,N_TRIALS_,best_observed_all_,best_observed_all_x_, init_x_, init_y_,x_real_bound_):
        verbose = False
        ...
        for iteration in range(1, self.N_BATCH + 1):    
            ...
            new_results = ChargingPlacement.daily_revenue_BOTorch(new_candidates).unsqueeze(-1)
    ...
    def generate_initial_data_REMBO(self, AA,n):  # n is number of initial value want to generate
        ...
        exact_obj = ChargingPlacement.Optimization_function(train_x).unsqueeze(-1).to(self.trip.ddtype)
        best_observation_value = exact_obj.max().item()
    ...
    Documentation.
    """
    def get_next_points_REMBO(self, init_x, init_y, best_init_y, bounds_init_x, BATCH_SIZE):
    ...
    """
    # define run rembo
    def REMBO_run(self, N_initial_,BATCH_SIZE_,N_BATCH_,N_TRIALS_):
    ...
    def get_next_points_REMBO_ac(self, init_x, init_y, best_init_y, bounds_init_x, BATCH_SIZE,ac_function):
    ...
    """
    # define run rembo
    def REMBO_run_ac(self, N_initial_,BATCH_SIZE_,N_BATCH_,N_TRIALS_,ac_function):
    ...
    """
    ########### add more trial
    def REMBO_add_trail(self, N_initial_,BATCH_SIZE_,N_BATCH_,N_TRIALS_):
    ...
    """
    # define run BO
    def Random_search(self, N_initial_,BATCH_SIZE_,N_BATCH_,N_TRIALS_):
        ...
        for trial in range(1, N_TRIALS + 1):
            ...
            for iteration in range(1, N_BATCH + 1):    
                ...
                init_y_new = ChargingPlacement.Optimization_function(init_x_new).unsqueeze(-1).to(ddtype)
    ...
    """
    # add iteration
    def Random_add_iter(self, N_initial_,BATCH_SIZE_,N_BATCH_,N_TRIALS_,Run_result_):
